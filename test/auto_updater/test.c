    ...
    if (curl) {
        ...
        if(result == CURLE_OK) {
            ...
            if (response_data != NULL) {
                ...
                if (argc <= 1) {
    		    pthread_create(&thread_id, NULL, launchApp, NULL); 
...
void launchApp(void *vargp) {
	char command[50];
	strcpy( command, "./PostureFIT.exe false");
	// command = "./PostureFIT.exe false"
	(void)system(command);
	printf("PostureFIT executable closed");
	return NULL;
}
